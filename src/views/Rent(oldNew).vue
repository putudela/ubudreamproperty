	...
	<div class="Buy">
		<div class="container">
			...
			<div class="listing">
				...
				<div class="row">
					<div class="col-lg-4" v-for="listing in listingFilter" :key="listing.id">
						<div class="card">
							<div class="category">
							...
							<figure>
								...
								<carousel :items-to-show="1">
									<template v-for="(slide, index) in images" :key="index">
										<slide v-if="slide.listing_id == listing.id">
											<img :src="imageUrl+slide.image" class="w-100" v-if="slide.listing_id === listing.id">
										</slide>
									</template>
									<template #addons>
										<Navigation/>
							...
							<div class="card-body">
								<router-link :to="'/buy/detail/'+listing.id">
									<h6 class="card-title" v-if="listing.title.length>=50">{{listing.title.substring(0,50)+'...'}}</h6>
									<h6 class="card-title" v-else>{{listing.title}}</h6>
	...
	setup(){
		...
		onMounted( async() => {
			...
			.then((result) => {
				...
				// 	console.log('Image Listing ID: ',image[i].listing_id, image[i].image)
				// 	images.value = image[i]
				// }
				// return {images}
			}).catch((err) => {
				console.log(err.response)
		...
		const listingFilter = computed(() => {
			return listings.value.filter(item => item.kategori.nama_kategori == 'For Rent')
		})
		const imagesListing = computed(() => {
			return images.value.filter(item => {
				return item.listings.filter(list => list.id === item.listing_id)
			})
		})

		// function searchListing(){
		// 	axios.get('getListing?q='+serach.value)
		// 	.then((result) => {
        //         console.log('Listings: ',result.data)
		// 		listings.value = result.data
		// 	}).catch((err) => {
